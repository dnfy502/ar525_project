"""
MC-PILOT Config B-Strat: platform height +0.5 m, stratified exploration
AR525 Group-3, IIT Mandi

Same as Config B (z_release=1.0m, lM=1.90m) but uses Stratified_Throwing_Exploration
instead of random. [0, uM] is divided into Nexp equal bands; each exploration throw
draws speed from the next band. Guarantees the GP sees the full speed range.
"""
...
T      = 0.85     # simulation horizon (s) — ball lands in ~0.72s at max speed from z=1.0
lc     = 0.5      # cost lengthscale / success radius (m)
lm     = 0.75     # min target distance (m)
lM     = 1.90     # max target distance (m) — verified: max range from z=1.0 is ~1.97m with drag
...
# Log directory
# ---------------------------------------------------------------------------
log_path = "results_mc_pilot_b_strat/" + str(seed)
