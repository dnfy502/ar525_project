"""
MC-PILOT Config C-Strat: platform height +1.0 m, stratified exploration
AR525 Group-3, IIT Mandi

Same as Config C (z_release=1.5m, lM=2.15m) but uses Stratified_Throwing_Exploration.
[0, uM] divided into Nexp equal bands; each exploration throw draws from the next band.
"""
...
T      = 0.95     # simulation horizon (s) — ball lands in ~0.80s at max speed from z=1.5
lc     = 0.5      # cost lengthscale / success radius (m)
lm     = 0.75     # min target distance (m)
lM     = 2.15     # max target distance (m) — verified: max range from z=1.5 is ~2.23m with drag
...
# Log directory
# ---------------------------------------------------------------------------
log_path = "results_mc_pilot_c_strat/" + str(seed)
