"""
MC-PILOT Config E-Strat2: platform height -0.5 m, targeted stratified exploration
AR525 Group-3, IIT Mandi

Same as E-Strat but explores only in [u_min=2.5, uM=3.5] m/s instead of [0, uM].
Root cause of E-Strat failure: from z=0.0m at alpha=35deg, targets at [0.75, 1.10]m
require release speeds [~2.80, ~3.39] m/s (17% of [0, 3.5]). Standard 5-band
stratification put 4/5 throws below lm=0.75m, giving the GP no useful data.
Fix: restrict exploration to [2.5, 3.5] m/s so all 5 throws land in or near the
target zone.
"""
...
# Log directory
# ---------------------------------------------------------------------------
log_path = "results_mc_pilot_e_strat2/" + str(seed)
...
config_log = {
    ...
    "config": "E_strat2",
    "z_release": 0.0,
    "exploration": "stratified_targeted",
