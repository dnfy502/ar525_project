"""
MC-PILOT Config E-Strat4: platform height -0.5 m, lc=0.25, Nexp=10, targeted strat
AR525 Group-3, IIT Mandi

Root cause of E-Strat3 failure: tanh saturation at max speed (3.5 m/s).
With lc=0.5 the cost landscape is too flat — the entire target range (0.35m) is smaller
than lc, so max-speed (overshoot by ~0.3m) gives cost~0.25, indistinguishable from
near-optimal. The optimizer can't escape this local minimum.

Fix: lc=0.25m. Max-speed cost rises to ~0.69 (clearly worse), giving the optimizer
a strong gradient toward lower speeds. lc=0.25 is safe because Nexp=10 gives the GP
enough data (275 pts) to predict landing positions within ~0.2m before Trial 1.
"""
...
lc     = 0.25     # halved: lc=0.5 > target range(0.35m), max-speed sat at cost~0.25; 0.25 breaks it
...
# Log directory
# ---------------------------------------------------------------------------
log_path = "results_mc_pilot_e_strat4/" + str(seed)
