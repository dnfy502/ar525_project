"""
MC-PILOT Config E-Strat5: lengthscales_init=[0.15, 0.15], lc=0.25, Nexp=10, targeted strat
AR525 Group-3, IIT Mandi

Root cause of E-Strat4 failure: RBF policy blind to target differences.
With lengthscales_init=[1.0, 1.0] and Config E's target range of only 0.35m, the RBF
sensitivity between the nearest and farthest targets is:
  exp(-0.5 * (0.35/1.0)^2) = 0.941  <- all targets look IDENTICAL

The policy outputs phi(P) ≈ same vector for all P in the 0.35m arc, so
speed = squash(W * phi(P)) is essentially constant regardless of target.
Policy converges to a single max-speed value (bias from Gaussian particle tails
beyond lM) — explains the constant 1.156m landing distance across all trials.
The lengthscale gradient is also near-zero (same reason), so the optimizer cannot
self-correct during training.

Compare with Config A (range 1.0m, ls=1.0m): sensitivity = 0.607 — targets
are meaningfully distinguishable. Config A works; Config E does not.

Fix: reduce lengthscales_init to 0.15m, giving sensitivity 0.066 between
extremes of Config E's arc — same resolution as Config A. The policy can now
map near targets -> low speed, far targets -> high speed.
"""
...
lc     = 0.25     # halved: lc=0.5 > target range(0.35m); lc=0.25 creates clear gradient
...
# ---------------------------------------------------------------------------
# Control policy — smaller lengthscales to match Config E's 0.35m target range
# ---------------------------------------------------------------------------
...
lengthscales_init = np.array([0.15, 0.15])   # KEY FIX: was [1.0, 1.0]
...
# Log directory
# ---------------------------------------------------------------------------
log_path = "results_mc_pilot_e_strat5/" + str(seed)
