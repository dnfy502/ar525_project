"""
MC-PILOT PBE Config B: PyBullet arm, z_release=1.0m, stratified exploration.
AR525 Group-3, IIT Mandi

Identical parameters to mc-pilot-elevated/test_mc_pilot_b_strat.py.
Physics provided by KUKA iiwa7 in PyBullet (arm base at z=0.5m so EE reaches z=1.0).
Joint velocity limits scaled by 1.5x to allow uM=3.5 m/s EE speed in simulation.
Ball velocity set explicitly at release — arm motion is cosmetic / for visualisation.

Usage:
  python test_mc_pilot_pbe_B.py -seed 1 -num_trials 10
"""
...
# CLI
# ---------------------------------------------------------------------------
p = argparse.ArgumentParser("mc-pilot-pb-elevated Config B")
...
# ---------------------------------------------------------------------------
# Parameters (identical to mc-pilot-elevated/test_mc_pilot_b_strat.py)
# ---------------------------------------------------------------------------
Nexp   = 5
Nopt   = 1500
M      = 400
...
RELEASE_POS  = np.array([0.0, 0.0, 1.0])
ARM_BASE_POS = (0.0, 0.0, 0.5)   # pedestal: EE neutral ~0.5m above base → z≈1.0
...
throwing_system = PyBulletThrowingSystem(
    ...
    vel_limit_multiplier=1.5,   # unlocks ~3.5 m/s EE speed; sim-only
)
...
# Log directory
# ---------------------------------------------------------------------------
log_path = f"results_mc_pilot_pbe_B/{seed}"
...
# ---------------------------------------------------------------------------
config_log = {
    "seed": seed, "config": "PBE_B",
    "simulator": "PyBullet", "arm_noise": "None",
    "z_release": 1.0, "arm_base": list(ARM_BASE_POS),
...
# Run
# ---------------------------------------------------------------------------
print(f"\nPBE-B: z_release=1.0m, uM={uM}, lm={lm}, lM={lM}, seed={seed}")
