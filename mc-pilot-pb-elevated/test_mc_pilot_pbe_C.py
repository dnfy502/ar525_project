"""
MC-PILOT PBE Config C: PyBullet arm, z_release=1.5m, stratified exploration.
AR525 Group-3, IIT Mandi

Identical parameters to mc-pilot-elevated/test_mc_pilot_c_strat.py.
Physics provided by KUKA iiwa7 in PyBullet (arm base at z=1.0m so EE reaches z=1.5).
Joint velocity limits scaled by 1.5x to allow uM=3.5 m/s EE speed in simulation.

Usage:
  python test_mc_pilot_pbe_C.py -seed 1 -num_trials 10
"""
...
# CLI
# ---------------------------------------------------------------------------
p = argparse.ArgumentParser("mc-pilot-pb-elevated Config C")
...
# ---------------------------------------------------------------------------
# Parameters (identical to mc-pilot-elevated/test_mc_pilot_c_strat.py)
# ---------------------------------------------------------------------------
Nexp   = 5
Nopt   = 1500
M      = 400
...
RELEASE_POS  = np.array([0.0, 0.0, 1.5])
ARM_BASE_POS = (0.0, 0.0, 1.0)   # pedestal: EE neutral ~0.5m above base → z≈1.5
...
# Log directory
# ---------------------------------------------------------------------------
log_path = f"results_mc_pilot_pbe_C/{seed}"
...
# ---------------------------------------------------------------------------
config_log = {
    "seed": seed, "config": "PBE_C",
    "simulator": "PyBullet", "arm_noise": "None",
    "z_release": 1.5, "arm_base": list(ARM_BASE_POS),
    "vel_limit_multiplier": 1.5,
...
# Run
# ---------------------------------------------------------------------------
print(f"\nPBE-C: z_release=1.5m, uM={uM}, lm={lm}, lM={lM}, seed={seed}")
