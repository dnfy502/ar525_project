"""
Collect a paper-friendly slip-noise sweep for mc-pilot-pybullet.

This is a thin, clearer entry point over run_pb_noise_sweep.py so slip-only
experiments can be launched with explicit defaults for the paper.

Examples:
  python collect_pb_slip_sweep.py --run
  python collect_pb_slip_sweep.py --run --seeds 1,2 --slip_alphas 0.05,0.10,0.15,0.20
  python collect_pb_slip_sweep.py --analyze_only
"""
...
def main():
    parser = argparse.ArgumentParser("Collect slip-noise sweep results")
    ...
    parser.add_argument("--slip_alphas", type=str, default="0.05,0.10,0.15,0.20")
    ...
    parser.add_argument("--output_dir", type=str, default="results_mc_pilot_pb_slip_sweep")
