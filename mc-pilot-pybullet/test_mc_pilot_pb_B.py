"""
MC-PILOT Config PB-B: PyBullet arm + VelocitySlipNoise — learnable noise demo.
AR525 Group-3, IIT Mandi

VelocitySlipNoise: v_actual = (1 - alpha) * v_cmd + N(0, sigma^2)

The multiplicative (1-alpha) term is the key difference from VelocityBiasNoise:
  - The loss scales with throw speed, so long throws (far targets) lose more
  - The optimal policy must output v_needed / (1-alpha) — a consistent multiplier
  - A naive policy (no noise awareness) systematically undershoots, worse at range
  - This IS demonstrably learnable from data

Noise-aware vs naive comparison:
  -noise_aware 1  (default): RL learns the (1-alpha) multiplier → hits rate improves
  -noise_aware 0            : policy ignores slip → systematic undershoot

Usage:
  python test_mc_pilot_pb_B.py -seed 1 -num_trials 10
  python test_mc_pilot_pb_B.py -seed 1 -num_trials 10 -alpha 0.15 -sigma 0.04
  python test_mc_pilot_pb_B.py -seed 1 -num_trials 10 -noise_aware 0
"""
...
from robot_arm.noise_models import VelocitySlipNoise

# ---------------------------------------------------------------------------
# CLI
# ---------------------------------------------------------------------------
p = argparse.ArgumentParser("mc-pilot-pybullet Config B — VelocitySlipNoise")
p.add_argument("-seed",        type=int,   default=1)
p.add_argument("-num_trials",  type=int,   default=10)
p.add_argument("-alpha",       type=float, default=0.20,
               help="fraction of v_cmd lost at release (gripper slip)")
p.add_argument("-sigma",       type=float, default=0.04,
               help="additive scatter std dev (m/s)")
...
# ---------------------------------------------------------------------------
# Parameters
# uM raised to 3.0 (from PB-A's 2.5) so the aware policy can compensate.
# With alpha=0.20, compensating for lM=1.1m requires v_cmd = 2.45/0.80 = 3.06 m/s.
...
# ---------------------------------------------------------------------------
# Noise model (shared between simulator and MC_PILOT when noise_aware=1)
# ---------------------------------------------------------------------------
arm_noise = VelocitySlipNoise(alpha=alpha, sigma=sigma, seed=seed + 100)
...
log_path  = f"results_mc_pilot_pb_B/alpha_{alpha_str}_{('aware' if noise_aware else 'naive')}/{seed}"
...
# ---------------------------------------------------------------------------
config_log = {
    "seed": seed, "config": "PB_B_velocity_slip",
    "simulator": "PyBullet", "arm_noise": "VelocitySlipNoise",
    "alpha": alpha, "sigma": sigma, "noise_aware": bool(noise_aware),
...
# Run
# ---------------------------------------------------------------------------
print(f"\nPB-B VelocitySlip: alpha={alpha}, sigma={sigma}, noise_aware={bool(noise_aware)}, seed={seed}")
